...
int scanfBuffer(const char* format, void* memoryAdress, const char* message)
{
	...
	int isScanned = 0; // Indicator with value 1 if the user input scanned succesfully, and 0 otherwise.
	int numOfBufferedChar; // Variable two store the number of characters in the input buffer.
	do
	{
		printf("%s", message);
		...
		// remained in the buffer (e.g., the input should be an integer but it was floating-point number)
		numOfBufferedChar = clearBuffer();

		if ((!isScanned) || (numOfBufferedChar > 0))
		{
			printf("Your given input was not succesfully parsed, please try again!\n");
		}
